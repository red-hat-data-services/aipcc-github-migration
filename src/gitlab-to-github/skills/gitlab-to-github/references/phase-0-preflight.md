...
If `.tekton/` is present, treat the repository as a possible Konflux-managed
container build. Its PipelineRun files are generated by the Product Management
Tool (PMT), not hand-written. This determines whether the Konflux/KRD
reconfiguration phase should be enabled.
...
If `detected.has_tekton` is true, ask the user to confirm before enabling the
Konflux/KRD reconfiguration phase. Do not silently enable it: this phase can
delete and recreate production Konflux Components in `konflux-release-data`.

```
⚠ This repo appears to be a Konflux-managed container build (its .tekton/
  files are PMT-generated PipelineRuns).

  Migrating it involves extra steps beyond a standard repo migration:
  regenerating .tekton/KRD resources via PMT, and reconfiguring its
  Konflux Component/ImageRepository resources in konflux-release-data
  to point at GitHub.

  Include the Konflux/KRD reconfiguration phase? [Y/n]
```

Record the answer as `konflux_managed` in the manifest. Default to yes,
matching detection, but always ask. If declined, or if `detected.has_tekton`
is false, skip the `konflux_krd` phase entirely.
